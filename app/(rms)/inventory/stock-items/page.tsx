...
export default function StockItemsPage() {
      ...
      case "actions": return (
        <Dropdown placement="bottom-end">
          ...
          <DropdownMenu aria-label="Row actions">
            <DropdownItem key="view" startContent={<Package size={15} color="#8A7D72" />} onPress={() => s.setDetail(it)}>View details</DropdownItem>
  ...
  const renderCard = (it: StockItem) => {
    ...
    return (
      <div onClick={() => s.setDetail(it)} className="bg-white border border-[#EEEAE6] rounded-2xl p-[14px] cursor-pointer active:bg-warm-50">
  ...
  return (
    <>
      ...
      {s.editing !== undefined && <AddEditModal item={s.editing} onClose={() => s.setEditing(undefined)} onSave={s.save} />}
      {s.detail && <DetailDrawer item={s.detail} onClose={() => s.setDetail(null)} onEdit={(it) => { s.setDetail(null); s.setEditing(it); }} onDelete={(it) => s.setDel({ item: it })} />}
      {s.del && <DeleteModal label={"item" in s.del ? `"${s.del.item.name}"` : `${s.selected.length} stock items`} onClose={() => s.setDel(null)} onConfirm={() => s.doDelete((it) => `"${it.name}"`)} />}
    </>
  );
}
