...
export default function MenuSetsPage() {
  ...
  const openDetail = (m: MenuSet) => router.push(`/menu/menu-sets/${m.id}`);
    ...
    switch (key) {
      ...
      case "name":        return (
        <button onClick={() => openDetail(m)}
          className="text-[13.5px] font-semibold text-ink whitespace-nowrap hover:text-[#F15022] transition-colors">
          {m.name}
        </button>
      );
