    ...
    >
      ...
      <div className="mt-2">
        ...
        <div className="border border-warm-200 rounded-[12px] overflow-x-auto">
          <div
            className="grid gap-x-3 bg-cream border-b border-warm-200 px-4 py-[10px] text-[11px] font-bold text-warm-600 uppercase tracking-[0.04em] min-w-[1040px]"
          ...
          {rows.map((row, i) => {
            ...
            return (
              <div
                key={row.id}
                className="grid gap-x-3 px-4 py-3 items-center min-w-[1040px]"
                style={{
      ...
      <div className="mt-2">
        ...
        <div className="border border-warm-200 rounded-[12px] overflow-x-auto">
          ...
          {rows.map((row, i) => {
            const amount = (Number(row.qty) || 0) * (Number(row.rate) || 0);
            return (
              <div key={row.id} className="grid gap-x-3 px-4 py-3 items-center min-w-[1040px]"
                style={{
                  gridTemplateColumns: "40px 1.4fr 1.3fr 0.95fr 0.85fr 0.95fr 1fr 1.3fr 40px",
                  borderBottom: i === rows.length - 1 ? "none" : "1px solid #F4EFEB",
                }}>
